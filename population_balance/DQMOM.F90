...
module dqmom
  ...
  use vector_tools
  use spud
  use global_parameters, only: OPTION_PATH_LEN, FIELD_NAME_LEN, current_time
...
contains
  ...
  subroutine dqmom_PD_algorithm(state, i_pop)
    ...
    do i_node = 1, node_count(moments(1)%ptr)
       ...
       P(1,2) = 1.0       
       do i = 2, 2*n_abscissa
          P(i,2) = (-1)**(i-1)*(node_val(moments(i)%ptr, i_node)/node_val(moments(1)%ptr, i_node)) !this is the problem .... The reason why all the values are same
          !print*, "The Moments  ", P
       end do
    ...
    do i_state = 1, option_count("/material_phase")
       do i_pop = 1, option_count(trim(states(i_state)%option_path)//'/population_balance')
          ...
          do i_abscissa = 1, option_count(trim(option_path)//'/abscissa/scalar_field')
             ...
             ! Get minimum weight value from the diamond filedqmom_apply_min_weight
  ...
  subroutine dqmom_calculate_source_term_ele(abscissa, weight, s_weighted_abscissa, s_weight, &
    ...
    real, dimension(X%dim, ele_ngi(abscissa(1), ele)) :: X_at_quad
